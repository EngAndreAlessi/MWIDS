...
void remove_all_edges_of_node(struct Graph* graph, int v, int verbose, FILE* fptr); // Remove all edges of a node v, assumes graph is not empty and node exists
struct List_int* get_neighborhood(struct Graph* graph, int v, int verbose, FILE* fptr); // Get a list with the neighbors of a vertex v on a graph
struct List_int* get_closed_neighborhood(struct Graph* graph, int v, int verbose, FILE* fptr); // Get the closed neighborhood of a vertex v on a graph
void remove_node_graph(struct Graph* graph, int v, int verbose, FILE* fptr); // Remove a node from graph
void delete_graph(struct Graph* graph, int verbose, FILE* fptr); // Delete entire graph
...
struct Graph* copy_graph(struct Graph* graph, int verbose, FILE* fptr); // Copy graph
